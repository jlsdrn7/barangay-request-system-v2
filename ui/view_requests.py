...
from ttkbootstrap import Frame, Label, Button, Scrollbar, Entry, StringVar, Combobox
...
class ViewRequests(Frame):
    def __init__(self, master):
        ...
        Label(self, text="View Document Requests", font=("Segoe UI", 16, "bold")).pack(anchor=NW, pady=(0, 10))
    ...
    def edit_request(self):
        ...
        request_id, old_resident, old_type, old_purpose, old_status = values

        edit_win = tk.Toplevel(self)
        edit_win.title("Edit Request")
        edit_win.geometry("400x340")
        edit_win.grab_set()

        Label(edit_win, text="Resident Name").pack(pady=(10, 0))
        resident_var = StringVar(value=old_resident)
        Entry(edit_win, textvariable=resident_var, width=40).pack()

        Label(edit_win, text="Document Type").pack(pady=(10, 0))
        doc_var = StringVar(value=old_type)
        doc_dropdown = Combobox(edit_win, textvariable=doc_var, state="readonly", width=37)
        doc_dropdown["values"] = ["Barangay Clearance", "Certificate of Indigency", "Certificate of Residency"]
        doc_dropdown.pack()

        Label(edit_win, text="Purpose").pack(pady=(10, 0))
        purpose_var = StringVar(value=old_purpose)
        Entry(edit_win, textvariable=purpose_var, width=40).pack()
        ...
        Button(edit_win, text="Save Changes", bootstyle="success", command=save_changes).pack(pady=15)
    ...
    def print_certificate(self):
        ...
        conn.close()
        self.load_requests()
        messagebox.showinfo("Auto-Approved", f"Request #{request_id} has been marked as Approved.")
